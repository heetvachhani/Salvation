package com.salvation.interceptor;
...
public class LoggingRequestInterceptor implements ClientHttpRequestInterceptor {
	...
	private void logRequest(final HttpRequest request, final byte[] body) {
		...
		logRequest.append("]\n[HTTP HEADERS:").append(request.getHeaders().toString());
		logRequest.append("]\n[URI:").append(request.getURI().toString());
		if(request.getHeaders().getContentType() == null) {
			logRequest.append("]\n[REQUEST BODY:").append(new String(body)).append("]");	
		}else {
			if(request.getHeaders().getContentType().toString().equals(MediaType.APPLICATION_JSON_UTF8_VALUE)
					|| request.getHeaders().getContentType().toString().equals(MediaType.APPLICATION_JSON_VALUE)) {
				logRequest.append("]\n[REQUEST BODY:").append(new String(body)).append("]");	
			}else{
